...
class HomeworkMemberDaoImpl extends BaseDao implements HomeworkMemberDao
{
    ...
    {
        $sql = "SELECT * FROM {$this->table} WHERE user_id = ? AND homework_id = ? AND type = 'student' LIMIT 1";
        ...
        public function findTeacherHomeworkByUserId($user_id,$homework_id)
    {
        $sql = "SELECT * FROM {$this->table} WHERE user_id = ? AND homework_id = ? AND type = 'teacher' LIMIT 1";
        $result= $this->getConnection()->fetchAll($sql, array($user_id, $homework_id)) ? : null;
        if(!empty($result)){
            return $result[0];
        }else{
